				  ...
				  struct iw_range *iwrange, int has_iwrange)
{
	...
	case IWEVTXDROP:
		...
		if (mac_to_ip(&event->u.addr, &ip, this_host.devs[0].ifname) !=
		    0) {
		...
		if (rt)
			...
			DEBUG(LOG_DEBUG, 0, "no route for ip=%s",
			      ip_to_str(ip));
		break;

	default:
		DEBUG(LOG_DEBUG, 0, "(Unknown Wireless event 0x%04X)",
		      event->cmd);
	}
...
static inline void llf_handle_netlink_events(struct rtnl_handle *rth)
{
	while (1) {
		...
		while (amt >= (int)sizeof(*h)) {
			...
			if (l < 0 || len > amt) {
				fprintf(stderr,
					"%s: malformed netlink message: len=%d\n",
