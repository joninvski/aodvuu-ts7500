			   ...
			   struct in_addr ip_dst)
{
	...
	while (rerr->dest_count) {
		...
		if (rt && rt->state == VALID
			 ...
			 * of sense... */
			if (0
			    && (int32_t) rt->dest_seqno >
			    (int32_t) rerr_dest_seqno) {
				DEBUG(LOG_DEBUG, 0,
				      "Udest ignored because of seqno");
...
#ifdef NS_PORT
			interfaceQueue((nsaddr_t) udest_addr.s_addr,
				       IFQ_DROP_BY_DEST);
#endif
			/* Invalidate route: */
			if (!rerr->n) {
			...
			if (rt->nprec && !(rt->flags & RT_REPAIR)) {
				...
				if (!new_rerr) {
					...
					new_rerr =
					    rerr_create(flags, rt->dest_addr,
							rt->dest_seqno);
					DEBUG(LOG_DEBUG, 0,
			...
			DEBUG(LOG_DEBUG, 0, "Ignoring UDEST %s",
			      ip_to_str(udest_addr));
		}
		udest = RERR_UDEST_NEXT(udest);
		rerr->dest_count--;
