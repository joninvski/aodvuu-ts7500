...
{
	return __cmsg_nxthdr_fix(__msg->msg_control, __msg->msg_controllen,
				 __cmsg);
}
...
void NS_CLASS aodv_socket_init()
{
	...
	for (i = 0; i < MAX_NR_INTERFACES; i++) {
		...
		if (setsockopt(DEV_NR(i).sock, SOL_SOCKET, SO_BINDTODEVICE,
			       &ifr, sizeof(ifr)) < 0) {
			fprintf(stderr, "SO_BINDTODEVICE failed for %s",
				DEV_NR(i).ifname);
			perror(" ");
			exit(-1);
		}
		...
		if (setsockopt(DEV_NR(i).psock, SOL_SOCKET, SO_SNDBUF,
			...
			DEBUG(LOG_NOTICE, 0,
			      "Could not set send socket buffer size");
		}
	...
	if (ratelimit) {
		...
		case AODV_RREQ:
			if (num_rreq == (RREQ_RATELIMIT - 1)) {
				...
				memcpy(&rreq_ratel[num_rreq], &now,
				       sizeof(struct timeval));
				num_rreq++;
			}
			break;
		case AODV_RERR:
			if (num_rerr == (RERR_RATELIMIT - 1)) {
				...
				memcpy(&rerr_ratel[num_rerr], &now,
				       sizeof(struct timeval));
				num_rerr++;
			}
			break;
	...
	if (dst.s_addr == AODV_BROADCAST) {
		...
		if (retval < 0) {
			...
			alog(LOG_WARNING, errno, __FUNCTION__,
			     "Failed send to bc %s", ip_to_str(dst));
			return;
		}
#endif
		...
		if (retval < 0) {
			alog(LOG_WARNING, errno, __FUNCTION__,
			     "Failed send to %s", ip_to_str(dst));
			return;
		}
#endif
	}
