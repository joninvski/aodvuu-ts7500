			...
			int ttl, u_int8_t flags)
{
	...
	for (i = 0; i < MAX_NR_INTERFACES; i++) {
		if (!DEV_NR(i).enabled)
			continue;
		rreq =
		    rreq_create(flags, dest_addr, dest_seqno, DEV_NR(i).ipaddr);
		aodv_socket_send((AODV_msg *) rreq, dest, RREQ_SIZE, ttl,
				 &DEV_NR(i));
	}
}
...
void NS_CLASS rreq_forward(RREQ * rreq, int size, int ttl)
{
	...
	for (i = 0; i < MAX_NR_INTERFACES; i++) {
		if (!DEV_NR(i).enabled)
			continue;
		aodv_socket_send((AODV_msg *) rreq, dest, size, ttl,
				 &DEV_NR(i));
	}
}
			   ...
			   unsigned int ifindex)
{
	...
	while ((rreqlen - extlen) > RREQ_SIZE) {
		...
		default:
			alog(LOG_WARNING, 0, __FUNCTION__,
			     "Unknown extension type %d", ext->type);
			break;
		}
	...
	if (rev_rt == NULL) {
		DEBUG(LOG_DEBUG, 0,
		      "Creating REVERSE route entry, RREQ orig: %s",
		      ip_to_str(rreq_orig));
		...
		if (rev_rt->dest_seqno == 0 ||
			...
			rev_rt =
			    rt_table_update(rev_rt, ip_src, rreq_new_hcnt,
		...
		/* This is a out of draft modification of AODV-UU to prevent
		   ...
		   suggested by the RREQ. */
			...
			DEBUG(LOG_DEBUG, 0,
			      "Dropping RREQ due to reverse route mismatch!");
			return;
		}
#endif
	}
	...
	if (internet_gw_mode) {
		...
		case HOST_INET:
			...
			rrep =
			    rrep_create(0, 0, 0,
					DEV_IFINDEX(rev_rt->ifindex).ipaddr,
	...
	if (rreq_dest.s_addr == DEV_IFINDEX(ifindex).ipaddr.s_addr) {
		...
		if (rreq_dest_seqno != 0) {
			if ((int32_t) this_host.seqno <
			    (int32_t) rreq_dest_seqno)
		...
		if (fwd_rt && fwd_rt->state == VALID && !rreq->d) {
			...
			if (fwd_rt->flags & RT_INET_DEST) {
				...
				lifetime =
				    timeval_diff(&gw_rt->rt_timer.timeout,
						 &now);

				rrep =
				    rrep_create(0, 0, gw_rt->hcnt,
						gw_rt->dest_addr,
						gw_rt->dest_seqno,
						rev_rt->dest_addr, lifetime);

				ext =
				    rrep_add_ext(rrep, RREP_INET_DEST_EXT,
						 rrep_size,
						 sizeof(struct in_addr),
						 (char *)&rreq_dest);
			...
			if (fwd_rt->dest_seqno != 0 &&
			    (int32_t) fwd_rt->dest_seqno >=
			    (int32_t) rreq_dest_seqno) {
				lifetime =
				    timeval_diff(&fwd_rt->rt_timer.timeout,
						 &now);
				rrep =
				    rrep_create(0, 0, fwd_rt->hcnt,
						fwd_rt->dest_addr,
						fwd_rt->dest_seqno,
						rev_rt->dest_addr, lifetime);
			...
			if (rreq->g) {
				rrep =
				    rrep_create(0, 0, rev_rt->hcnt,
						rev_rt->dest_addr,
						rev_rt->dest_seqno,
						fwd_rt->dest_addr, lifetime);

				rrep_send(rrep, fwd_rt, rev_rt, RREP_SIZE);

				DEBUG(LOG_INFO, 0,
				      "Sending G-RREP to %s with rte to %s",
				      ip_to_str(rreq_dest),
				      ip_to_str(rreq_orig));
			}
			return;
				...
				struct ip_data *ipd)
{
	...
	if (src_entry)
		ttl =
		    (int)(max(rt->hcnt, 0.5 * src_entry->hcnt) + LOCAL_ADD_TTL);
	else
							  ...
							  u_int32_t rreq_id)
{
	...
	if ((rec =
	     (struct rreq_record *)malloc(sizeof(struct rreq_record))) ==
	    NULL) {
