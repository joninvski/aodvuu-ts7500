...
void load_modules(char *ifname)
{
	...
	if (stat("./kaodv.ko", &st) == 0)
		sprintf(buf, "/sbin/insmod kaodv.ko ifname=%s &>/dev/null",
			ifname);
	else if (stat("./kaodv.o", &st) == 0)
		sprintf(buf, "/sbin/insmod kaodv.o ifname=%s &>/dev/null",
			ifname);
	else
		sprintf(buf, "/sbin/modprobe kaodv ifname=%s &>/dev/null",
			ifname);
...
void host_init(char *slave_ifname)
{
	char buf[1024], tmp_ifname[IFNAMSIZ],
	    slave_ifnames[(IFNAMSIZ + 1) * MAX_NR_INTERFACES], *iface;
	struct ifconf ifc;
	...
	if (!slave_ifname) {
		...
		slave_ifname = tmp_ifname;

		alog(LOG_NOTICE, 0, __FUNCTION__,
		     "Attaching to %s, override with -i <if1,if2,...>.",
		     tmp_ifname);
	}
	
	load_modules(slave_ifname);

	strcpy(slave_ifnames, slave_ifname);
	...
	/* iface = strtok(ifname, ","); */
	iface = strtok(slave_ifname, ",");

	/* OK, now lookup interface information, and store it... */
	do {
		sprintf(ifreq.ifr_name, "aodv%d", this_host.nif);
		
		if (ioctl(if_sock, SIOCGIFINDEX, &ifreq) < 0) {
			alog(LOG_ERR, errno, __FUNCTION__,
			     "Could not get index of %s", iface);
			close(if_sock);
			exit(-1);
		}
		...
		strcpy(DEV_IFINDEX(ifreq.ifr_ifindex).ifname, ifreq.ifr_name);
		strcpy(DEV_IFINDEX(ifreq.ifr_ifindex).slave_ifname, iface);

		/* Get IP-address of interface... */
		/* ina = get_if_info(iface, SIOCGIFADDR); */
/* 		if (ina == NULL) */
/* 			exit(-1); */
		inet_aton("192.168.23.15", &DEV_IFINDEX(ifreq.ifr_ifindex).ipaddr);
		inet_aton("192.168.23.255", &DEV_IFINDEX(ifreq.ifr_ifindex).broadcast);
		nl_set_ifaddr(DEV_IFINDEX(ifreq.ifr_ifindex).ipaddr, DEV_IFINDEX(ifreq.ifr_ifindex).broadcast, ifreq.ifr_ifindex);
...
int main(int argc, char **argv)
{
	...
	while (1) {
		...
		opt =
		    getopt_long(argc, argv, "i:fjln:dghoq:r:s:uwxDLRV",
				longopts, 0);
	...
	if (wait_on_reboot) {
		timer_init(&worb_timer, wait_on_reboot_timeout,
			   &wait_on_reboot);
