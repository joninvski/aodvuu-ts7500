...
char *NS_CLASS devs_ip_to_str()
{
	...
	for (i = 0; i < MAX_NR_INTERFACES; i++) {
		if (!DEV_NR(i).enabled)
			continue;
		index +=
		    sprintf(buf + index, "%s,", ip_to_str(DEV_NR(i).ipaddr));
	}
	str = buf;
	return str;
}
...
void NS_CLASS print_rt_table(void *arg)
{
	...
	len +=
	    sprintf(rt_buf,
		    ...
		    rt_tbl.num_entries, rt_tbl.num_active);
	len +=
	    sprintf(rt_buf + len,
		...
		list_foreach(pos, &rt_tbl.tbl[i]) {
			...
			if (list_empty(&rt->precursors))
				len += sprintf(rt_buf + len,
					       ...
					       rt->hcnt,
					       state_to_str(rt->state),
					       seqno_str,
					       (rt->hcnt ==
						255) ? 0 : timeval_diff(&rt->
									rt_timer.
									timeout,
									&now),
					       rt_flags_to_str(rt->flags),
					       if_indextoname(rt->ifindex,
							      ifname));
