...
long NS_CLASS hello_jitter()
{
	if (hello_jittering) {
#ifdef NS_PORT
		return (long)(((float)Random::integer(RAND_MAX + 1) / RAND_MAX -
			       0.5)
			      * JITTER_INTERVAL);
#else
		return (long)(((float)random() / RAND_MAX -
			       0.5) * JITTER_INTERVAL);
#endif
	} else
		return 0;
...
void NS_CLASS hello_send(void *arg)
{
	...
	if (time_diff >= HELLO_INTERVAL) {
		...
		for (i = 0; i < MAX_NR_INTERFACES; i++) {
			...
			aodv_socket_send((AODV_msg *) rrep, dest, msg_size, 1,
					 &DEV_NR(i));
		}
...
void NS_CLASS hello_process(RREP * hello, int rreplen, unsigned int ifindex)
{
	...
	while (rreplen > (int)RREP_SIZE) {
		...
		case RREP_HELLO_NEIGHBOR_SET_EXT:
			...
			for (i = 0; i < ext->length; i = i + 4) {
				ext_neighbor.s_addr =
				    *(in_addr_t *) ((char *)AODV_EXT_DATA(ext) +
						    i);

				if (ext_neighbor.s_addr ==
		...
		default:
			alog(LOG_WARNING, 0, __FUNCTION__,
			     "Bad extension!! type=%d, length=%d", ext->type,
			     ext->length);
			ext = NULL;
			break;
		}
	...
	if (!rt) {
		...
		rt = rt_table_insert(hello_dest, hello_dest, 1,
				     hello_seqno, timeout, state, flags,
				     ifindex);
		...
		if (flags & RT_UNIDIR) {
			...
			DEBUG(LOG_INFO, 0, "%s new NEIGHBOR!",
			      ip_to_str(rt->dest_addr));
		}
		rt->hello_cnt = 1;
